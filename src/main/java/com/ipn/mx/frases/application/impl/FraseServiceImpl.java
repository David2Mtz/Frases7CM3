...
public class FraseServiceImpl implements FraseService {
    ...
    public ByteArrayInputStream exportarFrase(List<Frase> frases) {
        ...
        try {
            ...
            PdfPTable table = new PdfPTable(3); // Fixed from 5 to 3 as there are only 3 columns
    ...
    @Override
    @Transactional(readOnly = true)
    public Frase findRandom() {
        return dao.findRandom();
    }

    @Override
    public void enviarFrasePorEmail(String destinatario) {
        Frase frase = dao.findRandom();
        if (frase != null) {
            String mensaje = String.format("<b>Frase del día:</b><br/><i>\"%s\"</i><br/>- %s",
                    frase.getTexto(), frase.getAutor());
            emailService.enviarCorreo(destinatario, "Frase Aleatoria", mensaje);
        }
    }
}
